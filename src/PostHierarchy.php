...
namespace Brisum\Wordpress\PostHierarchy;
...
class PostHierarchy
{
     ...
     * @param $assetsUri
     */
    public function __construct($postType, $assetsUri)
    {
    ...
    /**
     * Registration and init post type
     *
     * @return void
     */
    ...
    public function enqueueScripts()
    {
        ...
        if ($this->postType !== $screen->post_type || 'list' === $mode) {
            return;
        }
        ...
        ?>
            ...
            <?php foreach ($tree as $item) : ?>
                <li class="dd-item" data-id="<?php echo $item['post']->ID; ?>">
                    <div class="dd-handle">
                        <?php echo apply_filters('the_title', $item['post']->post_title) . " [{$item['post']->ID}: {$item['post']->post_name}]" ; ?>
                    </div>
